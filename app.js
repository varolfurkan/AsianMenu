...
buttonJapan.addEventListener("click", function() {
  filterItems("Japan");
});
...
function filterItems(category) {
  ...
  if (filteredItems.length > 0) {
    const filteredMenuItems = filteredItems.map(item => {
      return `  
    ...
    menuDiv.innerHTML = filteredMenuItems.join("");
  } else {
    menuDiv.innerHTML = "<p class='menu-items'>No items found in this category.</p>";
  }
}
